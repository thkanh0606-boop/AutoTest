...
class Header(QWidget):
    ...
    def __init__(self, store=None):
        ...
        self.website_combo.setFixedWidth(155)
        ...
        separator.setStyleSheet("color: #b6c0cc; font-size: 14px;")
        main_layout.addWidget(separator)

        page_layout = self._field_layout("TRANG ĐANG KIỂM THỬ")
        ...
        status_layout = self._field_layout("TRẠNG THÁI")
        self.status_button = QPushButton("●  Đã cấu hình")
        ...
        self.website_combo.currentIndexChanged.connect(self._load_pages)
        self.page_combo.currentIndexChanged.connect(self._emit_context)
    ...
    def reload_from_store(self):
        ...
        if self.store:
            self.store.reload()
            for website in self.store.websites():
                self.website_combo.addItem(website.get("name", ""), website.get("id"))
        else:
            self.website_combo.addItems(["StudyMate", "CoffeeGame"])
    ...
    def _load_pages(self):
        ...
        if self.store:
            website = self.store.find_website(self.website_combo.currentData())
            if website:
                for page in website.get("pages", []):
                    self.page_combo.addItem(page.get("name", ""), page.get("id"))
        else:
            self.page_combo.addItems(["Quản lý môn học", "Danh sách sinh viên", "Đăng nhập", "Trang chủ"])
    ...
    def _emit_context(self):
        if self.store:
            ...
            url = self.store.page_url(website_id, page_id)
        else:
            website_id = self.website_combo.currentText()
            page_id = self.page_combo.currentText()
    ...
    def current_context(self):
        return (
            ...
            self.page_combo.currentData() or "",
